...
def test_observer_attach_detach():
    ...
    subject.detach(ConcreteObserver(Mock())) 
    assert len(subject._observers) == 0
...
def test_observer_no_notification_if_detached():
    ...
    subject.change_state(20) # Change state after detach
    mock_update.assert_called_once() # Should still be 1 call, no new call after detach